...
public class ProcessInstanceTest {
	...
	public void statProcessInstance(){
		ProcessInstance pi = processEngine.getRuntimeService().startProcessInstanceByKey("myProcess", "businessKey")// 与正在执行的流程实例和执行对象相关的Service  
	            //.startProcessInstanceByKey("myProcess")
	            ;// 使用流程定义的key启动流程实例，key对应HelloWorld.bpmn文件中id的属性值，使用key值启动，默认是按照最新版本的流程定义启动
		System.out.println("流程实例ID："+pi.getId());//流程实例ID 57501 流程实例ID：myProcess:1:55004
	...
	public void findMyPersonaltask(){
		...
		//TaskQuery tqA=tq.taskUnassigned()//张三指定个人任务查询，指定办理人,办理人为空的时候未派职务的
				//.taskUnnassigned();
		TaskQuery tqA=tq.taskAssignee("赵六");
